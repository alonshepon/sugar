#a program to read and compute global sugar cane sugar beet from 1960 in comparison to staple crops
...
g<-ggplot(production,aes(x=Year,y=Value, fill = Item, color=Item))+geom_area()+#scale_fill_manual(values=c("#999999", "#E69F00"))+
  ...
  geom_text(x=2013, y=1600, label="sugarcane",color="lavenderblush4")+
  #geom_text(x=2013, y=2250, label="sugar beet", color="cornsilk4")+
  geom_text(x=2000, y=1430, label="sugar beet", color="cornsilk4")+
  geom_text(x=2013, y=2300, label="total sugar crops", color="cornsilk3")+
